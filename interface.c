...
void fenetre_draw(SDL_Surface *ecran){
    ...
    TTF_Font *police_T1=NULL, *police_T2=NULL, *police_T3=NULL;
    police_T1 = TTF_OpenFont("angelina.TTF", 72);
    police_T2 = TTF_OpenFont("angelina.TTF", 40);
    ...
    afficher_txt(step1, "-Dessinez le chiffre", 345,160, police_T3, color_blanc,ecran);
    afficher_txt(step2, "- Clic droit : effacer",345,200, police_T2,color_blanc,ecran);
    afficher_txt(step3,"-Entree : lancer le programme",345,240,police_T3, color_blanc, ecran);
    afficher_txt (resultat_txt, "Resultat",345,300,police_T2, color_blanc,ecran);
    resultat=afficher_txt(resultat, "?", 355, 360,police_T2,color_blanc,ecran);
    ...
    //afficher_txt(resultat, "?", 355, 360, police_T1, color_noire,ecran);
    resultat=afficher_txt(resultat, c, 355, 360, police_T2, color_blanc, ecran);
...
void fenetre_img(SDL_Surface *ecran){
    ...
    while(continuer==0){
       ...
       // wait_for_keypressed();
        ...
        new_image(img,result);
        for (int i = 0; i < c; i++)
        {
            char *c = int_to_char(result[i]);
...
void fenetre_train(SDL_Surface *ecran){
    ...
    afficher_txt(idk, "L'entrainement peut mettre du temps", 50, 170, police_T3, color_noire, ecran);
    afficher_txt(idk, "Le training est terminé", 50, 170, police_T3, color_blanc, ecran);
    //free
    free_test(test);
    free(label_test);
    free_train(train);
        ...
        switch (event.type){
            ...
            case SDL_MOUSEBUTTONUP:
                ...
                if (event.button.y > 450 && event.button.y <= 500 && event.button.x > 0 && event.button.x <= 180 ){  //RETOUR
                    ...
                    afficher_txt(msg,"Le train est lance",150,300,police_T2,color_blanc,ecran);
                    SDL_Flip(ecran);
